...
func ConvertXML(raw, datatype):
	var TriggerNode = DaBa.Tbl[datatype].get("XML-ID")
	var do
	if DaBa.UseCache:
		...
		if DaBa.Tbl[datatype].get("URL-EXT"):
			print(filename)
			filename = filename.format(Network.formatas)
			print(filename)
		var tabla = {"Network": Network.currenthost, "Filename": filename}
	...
	while xml.read() != ERR_FILE_EOF:
		...
		match checkdata:
			...
			6:
				pass
	DaBa.Tbl[datatype]["data"] = stringtionary
	if DaBa.UseCache:
		var savecache = FileAccess.open("user://"+Network.currenthost+"/"+DaBa.Tbl[datatype].get("Filename")+".json", FileAccess.WRITE_READ)
...
func ConvertXMLFile(filename, datatype):
	...
	while xml.read() != ERR_FILE_EOF:
		...
		match checkdata:
			...
			6:
				...
				pass
	if DaBa.UseCache:
		var savecache = FileAccess.open("user://"+Network.currenthost+"/"+DaBa.Tbl[datatype].get("Filename")+"full.json", FileAccess.WRITE_READ)
		savecache.store_string(JSON.stringify(stringtionary))
		print("XML processed")
	Satellite.emit_signal.call_deferred("LoadInfo")
